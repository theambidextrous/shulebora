            ...
            <div class="d-flex mt-2 justify-content-end">
                <div class="d-flex mr-3 ml-2">
                    <div class="chart-text mr-2">
                        <a href="#" data-toggle="modal" data-target="#new_timetable_modal" class="btn btn-info"><i class="mdi mdi-library-plus"></i> Add To Timetable</a>
                    </div>
                </div>
            </div>
        </div>
    ...
    <div class="container-fluid">
                ...
                <div class="card blog-widget">
                    <div class="card-body">
                        <div class="d-flex flex-wrap">
                            <div><h3 class="card-title">{{$p_title??'All Timetable Items'}}</h3></div>
                        </div>
                        ...
                        <div class="row">
                            ...
                            <div class="card-body" style="width:100%!important;">
                                <h6 class="card-subtitle">List of all timetable items across all subjects</a></h6>
                                <div class="table-responsive">
                                    <table style="width:100%!important;" id="file_export" class="table table-striped table-bordered display">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                <td>{{\App\User::find($timetable['created_by'])->name}}</td>
                                                <td>{{\App\Subject::find($timetable['created_by'])->name}}</td>
                                                <td>{{\App\User::find($timetable['teacher'])->name}}</td>
                                                <td>{{$timetable['date']}}</td>
                                                <td>{{$timetable['time']}}</td>
                                                <td><a href="{{ url('shule/bora/admin/timetables/' . $timetable['id']) }}" class="btn btn-link"><i class="mdi mdi-table-edit"></i>Manage</a></td>
                                            </tr>
                                            @endforeach
                                            @endif
                                        </tbody>
        ...
        <div id="new_timetable_modal" class="modal fade" tabindex="-1" role="dialog" aria-hidden="true">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-body">
                        ...
                        <form class="mt-4" method="POST" action="{{ route('admin.add_timetable') }}">
                            @csrf
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" id="date" data-toggle="tooltip"
                                    data-placement="bottom" title="date here" value="{{ old('date') }}" required autocomplete="date">
                                <span class="bar"></span>
                                @error('date')
                            ...
                            <div class="form-group mb-4">
                                ...
                                <input type="time" name="time" class="form-control @error('time') is-invalid @enderror" id="time" data-toggle="tooltip"
                                    data-placement="bottom" title="time here" value="{{ old('time') }}" required autocomplete="time">
                                <span class="bar"></span>
                                @error('time')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
