...
<div class="container-fluid">
    ...
    <div class="row">
        ...
        <div class="col-lg-7 col-xlg-7 col-md-7">
            <div class="card">
                <a class="nav-link active tm-lg" id="pills-timeline-tab" data-toggle="pill" href="#current-month" role="tab" aria-controls="pills-timeline" aria-selected="true"><i class="icon-book-open"></i> Revision Papers</a>
                <div id="accordion" class="custom-accordion mb-4 accordion-class">
                <div class="card-body">
                @if(count($papers))
                @php($ll = 1)
                @foreach( $papers as $paper )
                    <h4>({{$ll}}) {{$paper['title']}}</h4>
                    <a class="btn btn-link" target="_blank" href="{{route('admin.securefinder', ['file' => $paper['file_content']])}}">
                    <h6><i class="icon-arrow-down-circle"></i> Download This Paper</h6></a>
                    <small class="text-muted">
                    <i class="icon-book-open"></i> {{App\Subject::find($paper['subject'])->name}} 
                    <i class="icon-clock"></i> {{ date('M jS, Y', strtotime($paper['created_at'])) }}
                    </small>
                    <hr>
                    @php($ll++)
                @endforeach
